...
function normalizeCanonicalUrl(inputUrl) {
  ...
  const fullUrl = source.startsWith('http')
    ? source
    : `${baseUrl}${source.startsWith('/') ? source : `/${source}`}`;
...
export default function useSeo({
  ...
  useEffect(() => {
    ...
    setMeta(
      ...
      'content',
      schemaType === 'Article' ? 'article' : 'website'
    );
    ...
    // Remove old JSON-LD before creating new schema
    setJsonLd(null);
    ...
    if (schemaType === 'Article') {
      const articleSchema = {
        ...
        image: normalizeImageUrl(
          schemaData?.image || image || DEFAULT_IMAGE
        ),
        ...
        articleSection:
          schemaData?.articleSection || 'Government Jobs',
      };
    ...
    return () => {
      // No cleanup required because the next page updates the tags.
    };
  }, [
    title,
    description,
